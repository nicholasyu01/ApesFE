...
const EnhancedTableToolbar = (props) => {
    ...
    const { numSelected, deleteWebsites } = props;
...
export default function WebsitesTable2(props) {
    ...
    const { websites, deleteWebsites, selected, setSelected, heads } = props;
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = websites?.map((n) => n);
    ...
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, websites?.length - page * rowsPerPage);
